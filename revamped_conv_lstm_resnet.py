...
def load_frames(points, labels):
  ...
  for point, label in zip(points, labels):
    ...
    for file_path in point:
      '''
      tf_filepath = tf.convert_to_tensor(file_path, dtype=tf.string)
      tf_img_str = tf.read_file(tf_filepath)
      tf_img = tf.image.decode_jpeg(tf_img_str, channels=3)
      #preprocess = vgg_preprocessing.preprocess_image(tf_img, VGG_INPUT_SIZE, VGG_INPUT_SIZE, is_training=False)
      #processed = tf.expand_dims(preprocess, 0)
      #np_image = scipy.ndimage.imread(file_path)
      np_images.append(tf.image.resize_images(tf.image.convert_image_dtype(tf_img, dtype=tf.float32), [128,128]).eval())
      '''
      np_image = scipy.ndimage.imread(file_path)
      np_images.append(transform.resize(np_image, [128,128]))
...
def network(inputs, hidden):
  '''
  with tf.name_scope('mobilenet_feat/'):
    module = hub.Module("https://tfhub.dev/google/imagenet/mobilenet_v2_035_128/feature_vector/1")
    features = module(inputs)
    print(features)
  #_, end_points = mobilenet_v1.mobilenet_v1(inputs, num_classes=1000, is_training=False)
  '''
...
#network_template = tf.make_template('network_feat/', network)
...
def train():
  ...
  with tf.Graph().as_default():
    ...
    for i in range(FLAGS.seq_length):
        module = hub.Module("https://tfhub.dev/google/imagenet/mobilenet_v2_035_128/feature_vector/1")
        x_unwrap.append(module(x_dropout[:,i,:,:,:]))
    ...
    #transposed = tf.transpose(x_unwrap, [1,0,2,3,4])
    transposed = tf.transpose(x_unwrap, [1,0,2])
    #print('transposed:')
    #print(transposed)
    rnn_cell = tf.nn.rnn_cell.BasicRNNCell(512)
    initial_state = rnn_cell.zero_state(16, dtype=tf.float32)
    reshaped1 = tf.reshape(transposed, [16, 10, 1280]) 
    outputs, state = tf.nn.dynamic_rnn(rnn_cell, reshaped1, initial_state=initial_state, dtype=tf.float32)
    ...
    loss_vector = tf.nn.softmax_cross_entropy_with_logits(labels=y, logits=fully_connected_output)
    loss = tf.reduce_mean(loss_vector)
    tf.summary.scalar('loss', loss)
    correct_pred = tf.equal(tf.argmax(fully_connected_output, 1), tf.argmax(y, 1))
   ...
   # training
    ...
    for step in xrange(FLAGS.max_step):
      ...
      while index + 16 > len(x_train):
          index = (step * 16) % len(x_train)
      #index = random.randint(0, len(x_train) - 1)
      features, labels = load_frames(x_train[index:index + 16], y_train[index:index + 16])
      t = time.time()
      #print(features)
      #print(labels)
      _, loss_r = sess.run([train_op, loss],feed_dict={x:list(features), y:labels, keep_prob:FLAGS.keep_prob})
      elapsed = time.time() - t
      ...
      if step%200 == 9:
        ...
        for i in range(len(x_dev)/16):
            ...
            if random.uniform(0, 1) > SAMPLE_RATE:
                continue
            total += 1
            dev_features, dev_labels = load_frames(x_dev[begin:begin + 16], y_dev[begin:begin + 16])
    ...
    for i in range(len(x_test)/16):
        ...
        begin = i * 16
        total += 1
        test_features, test_labels = load_frames(x_test[begin:begin + 16], y_test[begin:begin + 16])
